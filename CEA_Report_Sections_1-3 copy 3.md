...
![Figure 1: High-Level Architecture Diagram - Multi-Tier Cloud Deployment](Figure 1.png)
...
![Figure 2: Network Topology - VPC Subnet Architecture](Figure 2.png)

### 1.6 Request-Response Data Flow

[FIGURE 3: Chat Streaming Request Flow Sequence]

![Figure 3: Chat Streaming Request Flow Sequence](Figure 3.png)
...
![Figure 4: Security Architecture and Trust Boundaries](Figure 4.png)

The architecture defines four security zones with clear trust boundaries, implementing defense-in-depth security principles:

1. **Public Zone (Trust Boundary 1)**: Internet-facing services (CloudFront, S3 Static Assets)
   - Trust Boundary: Between internet and AWS cloud
   - Protection: WAF, DDoS protection (AWS Shield), TLS/SSL encryption, geographic restrictions
   - Components: CloudFront CDN with integrated WAF, S3 bucket for static assets

2. **DMZ (Demilitarized Zone, Trust Boundary 2)**: Load balancers and NAT gateways in public subnets
   - Trust Boundary: Between public and application zones
   - Protection: Security groups, network ACLs, TLS termination, rate limiting
   - Components: Application Load Balancer, NAT Gateway for outbound connectivity

3. **Application Zone (Trust Boundary 3)**: Backend application servers in private subnets
   - Trust Boundary: Between application and data zones
   - Protection: IAM roles (least privilege), encrypted connections, JWT authentication, security groups, no direct internet access
   - Components: EC2 backend instances running FastAPI with Gunicorn

4. **Data Zone (Trust Boundary 4)**: Databases and storage in isolated private subnets
   - Trust Boundary: Most restricted zone, final security layer
   - Protection: Encryption at rest (AES-256), IAM policies, audit logging (CloudTrail), VPC isolation, strict security groups
   - Components: RDS PostgreSQL, S3 vector stores, AWS Secrets Manager (all encrypted)
...
![Figure 5: CI/CD Pipeline Flow Diagram](Figure 5.png)
